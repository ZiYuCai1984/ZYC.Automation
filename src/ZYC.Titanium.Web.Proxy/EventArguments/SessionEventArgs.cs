...
public class SessionEventArgs : SessionEventArgsBase
{
    ...
    public bool ReRequest
    {
        ...
        set
        {
            ...
            {
                throw new Exception("Response status code is empty. Cannot request again a request " +
    ...
    internal void OnMultipartRequestPartSent(ReadOnlySpan<char> boundary, HeaderCollection headers)
    {
        try
        {
            MultipartRequestPartSent?.Invoke(this,
                new MultipartRequestPartSentEventArgs(this, boundary.ToString(), headers));
        }
        catch (Exception ex)
        {
    ...
    internal async Task SyphonOutBodyAsync(bool isRequest, CancellationToken cancellationToken)
    {
        ...
        await reader.CopyBodyAsync(requestResponse, true, NullWriter.Instance, TransformationMode.None, isRequest, this,
            cancellationToken);
    ...
    /// </summary>
    /// <returns></returns>
    internal async Task CopyRequestBodyAsync(IHttpStreamWriter writer, TransformationMode transformation,
    ...
    private async Task CopyResponseBodyAsync(IHttpStreamWriter writer, TransformationMode transformation,
        CancellationToken cancellationToken)
    {
        var response = HttpClient.Response;
        await HttpClient.Connection.Stream.CopyBodyAsync(response, false, writer, transformation, false, this,
            cancellationToken);
        response.IsBodyReceived = true;
    }
    ...
    /// </summary>
    /// <returns></returns>
    private async Task<long> ReadUntilBoundaryAsync(ILineStream reader, long totalBytesToRead,
        ReadOnlyMemory<char> boundary, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (bytesRead < totalBytesToRead &&
                   (reader.DataAvailable || await reader.FillBufferAsync(cancellationToken)))
            {
