...
internal class Http2Helper
{
        ...
        ExceptionHandler? exceptionFunc)
    {
        ...
        while (true)
        {
            var read = await ForceRead(input, frameHeaderBuffer, 0, 9, cancellationToken);
            if (read != 9)
            {
                return;
            }
            ...
            if (type == Http2FrameType.Data && args != null)
            {
                ...
                rr = isClient ? args.HttpClient.Request : args.HttpClient.Response;

                var padded = (flags & Http2FrameFlag.Padded) != 0;
                var endStreamFlag = (flags & Http2FrameFlag.EndStream) != 0;
                if (endStreamFlag)
                {
                    endStream = true;
                }
            ...
            {
                var endHeaders = (flags & Http2FrameFlag.EndHeaders) != 0;
                var padded = (flags & Http2FrameFlag.Padded) != 0;
                var priority = (flags & Http2FrameFlag.Priority) != 0;
                var endStreamFlag = (flags & Http2FrameFlag.EndStream) != 0;
                if (endStreamFlag)
                {
                    endStream = true;
                ...
                if (type == Http2FrameType.PushPromise)
                {
                    ...
                    Debug.WriteLine("PROMISE STREAM: " + streamId + ", " + promisedStreamId +
                                    ", CONN: " + connectionId);
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        var statusHack = HttpHeader.Encoding.GetString(headerListener.Status.Span);
                        int.TryParse(statusHack, out var statusCode);
            ...
            else if (type == Http2FrameType.Settings)
            {
                ...
                while (pos < length)
                {
                    var identifier = (buffer[pos++] << 8) + buffer[pos++];
                    var value =
            ...
            {
                var errorCode = (buffer[0] << 24) + (buffer[1] << 16) + (buffer[2] << 8) + buffer[3];
                if (streamId == 0)
                {
                    // connection error
                    exceptionFunc?.Invoke(new ProxyHttpException("HTTP/2 connection error. Error code: " + errorCode,
                        null, args));
                    return;
                }
                ...
                {
                    exceptionFunc?.Invoke(new ProxyHttpException("HTTP/2 stream error. Error code: " + errorCode, null,
                        args));
                }
            }
            ...
            if (endStream && rr!.ReadHttp2BodyTaskCompletionSource != null)
            {
                if (!rr.BodyAvailable)
                {
                    ...
                    if (rr.ContentEncoding != null)
                    {
                        ...
                        {
                            using (var zip =
                                   DecompressionFactory.Create(
                                       CompressionUtil.CompressionNameToEnum(rr.ContentEncoding),
            ...
            {
                sessions.TryRemove(streamId, out _);
                Debug.WriteLine("REMOVED CONN: " + connectionId + ", CLIENT: " + isClient + ", STREAM: " + streamId +
                                ", TYPE: " + type);
            }
    ...
    private static async Task SendHeader(Http2Settings settings, Http2FrameHeader frameHeader, byte[] frameHeaderBuffer,
    ...
    private static async Task SendBody(Http2Settings settings, RequestResponseBase rr, Http2FrameHeader frameHeader,
        byte[] frameHeaderBuffer, byte[] buffer, Stream output)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (bytesToRead > 0)
        {
            var read = await input.ReadAsync(buffer, offset, bytesToRead, cancellationToken);
            if (read == 0)
            {
                break;
            }
    ...
    private class MyHeaderListener : IHeaderListener
    {
        ...
        public void AddHeader(ByteString name, ByteString value, bool sensitive)
        {
            if (name.Span[0] == ':')
            {
                var nameStr = Encoding.ASCII.GetString(name.Span);
                switch (nameStr)
                {
                    case ":method":
                        Method = value;
