...
internal class Encoder
{
        ...
        false, HpackUtil.IndexType indexType = HpackUtil.IndexType.Incremental, bool useStaticName = true)
    {
        ...
        if (sensitive)
        {
            var nameIndex = GetNameIndex(name);
        ...
        var headerSize = HttpHeader.SizeOf(name, value);
        ...
        if (headerField != null)
        {
            var index = GetIndex(headerField.Index) + StaticTable.Length;
        ...
        else
        {
            ...
            {
                var nameIndex = useStaticName ? GetNameIndex(name) : -1;
    ...
    {
        var huffmanLength = HuffmanEncoder.Instance.GetEncodedLength(stringData);
    ...
    private class HeaderEntry : HttpHeader
    {
        ...
        public HeaderEntry(int hash, ByteString name, ByteString value, int index, HeaderEntry? next) : base(name,
            value, true)
        {
            Index = index;
            Hash = hash;
